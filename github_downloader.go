...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("BHOPCORD_DEV_INSTALL") == "1"
	...
	BhopcordFile := BhopcordDirectory

	stat, err := os.Stat(BhopcordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		BhopcordFile = path.Join(BhopcordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(BhopcordFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Bhopcord Install. Checking for hash...")

	re := regexp.MustCompile(`// Bhopcord (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(BhopcordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", BhopcordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", BhopcordDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(BhopcordDirectory)
