...
var BhopcordDirectory string

func init() {
	if dir := os.Getenv("BHOPCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using BHOPCORD_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../BhopcordData")
		BaseDir = path.Join(dir, "..", "BhopcordData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("Bhopcord").UserConfig()
	}
	dir := os.Getenv("BHOPCORD_DIRECTORY")
	if dir == "" {
	...
	if dir != "" {
		Log.Debug("Using BHOPCORD_DIRECTORY")
		BhopcordDirectory = dir
	} else {
		BhopcordDirectory = path.Join(BaseDir, "bhopcord.asar")
	}
}
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := WriteAppAsar(appAsar, BhopcordDirectory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", BhopcordDirectory+"...")
		...
		args = append(args, "override", name, "--filesystem="+BhopcordDirectory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + BhopcordDirectory + ": " + err.Error())
		}
	}
	return nil
}
