...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/wiz772/Bhoplotl/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "BhoplotlCli.exe", "Bhoplotl.exe")
		return BaseUrl + filename
	case "darwin":
		switch runtime.GOARCH {
		case "amd64":
			return BaseUrl + "Bhoplotl-darwin-x64.zip"
		case "arm64":
			return BaseUrl + "Bhoplotl-darwin-arm64.zip"
		default:
			return ""
		}
	case "linux":
		return BaseUrl + "BhoplotlCli-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "BhoplotlUpdate")
	if err != nil {
