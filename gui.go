...
func main() {
	...
	win = g.NewMasterWindow("Bhoplotl", 1200, 800, linuxFlags)
...
func InstallLatestBuilds() (err error) {
	...
	if err != nil {
		ShowModal("Uh Oh!", "Failed to install the latest Bhopcord builds from GitHub:\n"+err.Error())
	}
	return
}
...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		renderErrorCard(
			...
			func() *g.MarkdownWidget {
				if cachedWarningMarkdown == nil {
					cachedWarningMarkdown = g.Markdown("**Github** is the only official place to get Bhopcord. Any other site claiming to be us is malicious.\n" +
			...
			g.Style().
				...
				To(
					g.Button("Reinstall / Repair").
						...
						Size(btnWidth, 50),
					Tooltip("Reinstall & Update Bhopcord"),
				),
			g.Style().
		...
		InfoModal("#patched", "Successfully Patched", "If Discord is still open, fully close it first.\n"+
			"Then, start it and verify Bhopcord installed successfully by looking for its category in Discord Settings"),
		...
		InfoModal("#scuffed-install", "Hold On!", "You have a broken Discord Install.\n"+
			...
			"You need to fix this before patching, otherwise Bhopcord will likely not work.\n\n"+
		...
		RawInfoModal("#openasar-confirm", "OpenAsar", "OpenAsar is an open-source alternative of Discord desktop's app.asar.\n"+
			"Bhopcord is in no way affiliated with OpenAsar.\n"+
	...
	g.SingleWindow().
		...
		Layout(
			g.Style().SetFontSize(baseFontSize).To(
				...
				g.Row(
					g.Label(Ternary(IsDevInstall, "Dev Install: ", "Bhopcord will be downloaded to: ")+BhopcordDirectory),
					g.Style().
						SetColor(g.StyleColorButton, DiscordBlue).
						SetStyle(g.StyleVarFramePadding, 4, 4).
						To(
							g.Button("Open Directory").OnClick(func() {
								g.OpenURL("file://" + path.Dir(BhopcordDirectory))
							}),
						),
				),

				&CondWidget{!IsDevInstall, func() g.Widget {
					return g.Label("To customise this location, set the environment variable 'BHOPCORD_USER_DATA_DIR' and restart me").Wrapped(true)
				}, nil},

				g.Dummy(0, 10),
				g.Label("Bhoplotl Version: "+buildinfo.InstallerTag+" ("+buildinfo.InstallerGitHash+")"+Ternary(IsSelfOutdated, " - OUTDATED", "")),
				g.Label("Local Bhopcord Version: "+InstalledHash),

				&CondWidget{
					GithubError == nil,
					func() g.Widget {
						if IsDevInstall {
							return g.Label("Not updating Bhopcord due to being in DevMode")
						}
						return g.Label("Latest Bhopcord Version: " + LatestHash)
					}, func() g.Widget {
